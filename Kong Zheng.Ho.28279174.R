...
# Check the proportion of Yes and No in RainToday and RainTomorrow columns
raintmr_freq_table = as.data.frame(xtabs(~ RainTomorrow, WAUS))
...
# Decision Tree #
#################
dt.fit = tree(RainTomorrow ~ ., data=WAUS.train)
summary(dt.fit)

###############
# Naive Bayes #
###############
nb.fit = naiveBayes(RainTomorrow ~ ., data=WAUS.train)
summary(nb.fit)
...
###########
set.seed(28279174)
bag.fit = bagging(RainTomorrow ~ ., data=WAUS.train, mfinal=5)
summary(bag.fit)
...
############
set.seed(28279174)
boost.fit = boosting(RainTomorrow ~ ., data=WAUS.train, Mfinal=5)
summary(boost.fit)
...
#################
set.seed(28279174)
rf.fit = randomForest(RainTomorrow ~ ., data=WAUS.train)
summary(rf.fit)
...
dt.cm = table(actual=WAUS.test$RainTomorrow, predicted=dt.predict) # confusion matrix
dt.cm 
...
nb.cm = table(actual=WAUS.test$RainTomorrow, predicted=nb.predict) # confusion matrix
nb.cm 
...
rf.cm = table(actual=WAUS.test$RainTomorrow, predicted=rf.predict) # confusion matrix
rf.cm 
...
for (i in 1:num_models) {
  pred = prediction(preds[,i*2], WAUS.test$RainTomorrow)
...
pruned_dt.cm = table(actual=WAUS.test$RainTomorrow, predicted=pruned_dt.predict) # confusion matrix
pruned_dt.cm 
...
set.seed(28279174)
new_rf.fit = randomForest(RainTomorrow ~ ., data=WAUS.train[,c(1:11, 13)], mtry = 7)

# Calculate accuracy
new_rf.predict = predict(new_rf.fit, WAUS.test)
new_rf.cm = table(actual=WAUS.test$RainTomorrow, predicted=new_rf.predict) # confusion matrix
new_rf.cm 
...
new_WAUS$NoRainTmr = new_WAUS$RainTomorrow == 0
new_WAUS$HasRainTmr = new_WAUS$RainTomorrow == 1
...
set.seed(28279174)
nn.fit = neuralnet(NoRainTmr + HasRainTmr ~ . - RainTomorrow, new_WAUS.train, hidden = 1, linear.output=F)
...
colnames(nn.predrdfs) = c("Obs", "RainTomorrow")
nn.predrdfs = nn.predrdfs[order(nn.predrdfs$Obs),]

# Create confusion matrix and calculate the accuracy
nn.cm = table(observed=new_WAUS.test$RainTomorrow, predicted=nn.predrdfs$RainTomorrow)
nn.cm
